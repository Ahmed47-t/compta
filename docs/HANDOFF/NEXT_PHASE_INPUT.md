...
6. **PoC 6 (DLG & PCC Read-Only Analysis):**
   - كتابة نص بايثون لقراءة وفحص بنية عينة ملفات `.DLG` و `.PCC` في وضع `Read-Only` دون تغيير مصدرها.
   - **تحذير إلزامي:** يُمنع افتراض أن الامتداد `.DLG` هو `DBF` أو أن `.PCC` هو `SDF` مسبقاً (وفق تحذير `PHASE-01.md` والمراجعة `PHASE-00-REVIEW.md`). يُحدَّد النوع الفعلي عبر الفحص الجنائي للعينات (Magic Bytes / Header Profiling)، وتُوثَّق النتيجة بدل افتراضها.
